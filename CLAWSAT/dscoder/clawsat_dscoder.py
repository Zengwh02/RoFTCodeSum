...
def run_CLAWSAT(args, model, tokenizer, data_A, data_B, data_C, optimizer, scheduler):
    '''
    Run CLAWSAT
    '''
    ...
    for epoch in range(args.epochs):
        ...
        for batch_A, batch_B, batch_C in tqdm(zip(data_A, data_B, data_C), total=len(data_A), position=0, leave=True,
            ...
            print(f"Loss: {loss_list}, LR: {current_lr:.2e}")
...
def main(args):
    ...
    run_CLAWSAT(args, model, tokenizer, data_loader_a, data_loader_b, data_loader_c, optimizer, scheduler)
